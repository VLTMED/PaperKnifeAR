  ...
  return (
    <NativeToolLayout title="PDF to Image" description="Convert document pages into high-quality images." actions={pdfData && !pdfData.isLocked && !downloadUrl && <ActionButton />}>
      ...
      {!pdfData ? (
        ...
        >
          <div className="w-20 h-20 bg-ember-50 dark:bg-ember-900/20 text-ember-500 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform"><ImageIcon size={32} /></div>
          <h3 className="text-xl font-bold dark:text-white mb-2">Select PDF</h3>
          <p className="text-sm text-obsidian-500">Tap to start converting</p>
        </button>
      ) : pdfData.isLocked ? (
        <div className="max-w-md mx-auto">
          <div className="bg-obsidian-50 dark:bg-obsidian-900 p-8 rounded-[2.5rem] border border-obsidian-200 dark:border-white/5 text-center">
            ...
            <input type="password" value={unlockPassword} onChange={(e) => setUnlockPassword(e.target.value)} placeholder="Password" className="w-full bg-obsidian-100 dark:bg-obsidian-950 rounded-2xl px-6 py-4 border border-transparent focus:border-ember-500 outline-none font-bold text-center mb-4" />
        ...
        <div className="space-y-6">
          <div className="bg-obsidian-50 dark:bg-obsidian-900 p-6 rounded-3xl border border-obsidian-200 dark:border-white/5 flex items-center gap-6">
            <div className="w-16 h-20 bg-obsidian-100 dark:bg-obsidian-950 rounded-xl overflow-hidden shrink-0 border border-obsidian-200 dark:border-obsidian-800 flex items-center justify-center text-ember-500">{pdfData.thumbnail ? <img src={pdfData.thumbnail} className="w-full h-full object-cover" /> : <ImageIcon size={20} />}</div>
          ...
          <div className="bg-obsidian-50 dark:bg-obsidian-900 p-8 rounded-[2rem] border border-obsidian-200 dark:border-white/5 space-y-8 shadow-sm">
            {!downloadUrl ? (
              <>
                <div><label className="block text-[10px] font-black uppercase text-obsidian-500 mb-4 tracking-widest px-1">Image Format</label><div className="grid grid-cols-2 gap-3">{(['jpg', 'png'] as const).map(fmt => <button key={fmt} onClick={() => setFormat(fmt)} className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center ${format === fmt ? 'border-ember-500 bg-ember-50/50 dark:bg-ember-900/10' : 'border-obsidian-200 dark:border-white/5'}`}><span className={`font-black uppercase text-[10px] ${format === fmt ? 'text-ember-500' : 'text-obsidian-500'}`}>{fmt}</span></button>)}</div></div>
                <div><label className="block text-[10px] font-black uppercase text-obsidian-500 mb-3 tracking-widest px-1">Output ZIP Name</label><input type="text" value={customFileName} onChange={(e) => setCustomFileName(e.target.value)} className="w-full bg-obsidian-100 dark:bg-obsidian-950 rounded-xl px-4 py-3 border border-transparent focus:border-ember-500 outline-none font-bold text-sm dark:text-white" /></div>
              </>
            ) : (
            ...
            <button onClick={() => { setPdfData(null); setIsProcessing(false); }} className="w-full py-2 text-[10px] font-black uppercase text-obsidian-400 hover:text-ember-500 transition-colors">Close File</button>
          </div>
        </div>
      )}
      <PrivacyBadge />
