...
export default function MetadataTool() {
  ...
  const ActionButtons = () => (
    <div className="flex flex-col gap-2">
       <button onClick={() => saveMetadata(false)} disabled={isProcessing} className={`w-full bg-ember-500 text-white font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 py-4 rounded-2xl text-sm md:p-6 md:rounded-3xl md:text-xl flex items-center justify-center gap-3 shadow-lg shadow-ember-500/20`}>
    ...
    <NativeToolLayout title="Metadata Editor" description="Edit or wipe document properties for better privacy." actions={pdfData && !pdfData.isLocked && !downloadUrl && <ActionButtons />}>
      ...
      {!pdfData ? (
        <div onClick={() => !isProcessing && fileInputRef.current?.click()} className="border-4 border-dashed border-obsidian-200 dark:border-obsidian-900 rounded-[2.5rem] p-12 text-center hover:bg-ember-50 dark:hover:bg-ember-900/10 transition-all cursor-pointer group">
          <div className="w-20 h-20 bg-ember-50 dark:bg-ember-900/20 text-ember-500 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform"><Edit3 size={32} /></div>
          <h3 className="text-xl font-bold dark:text-white mb-2">Select PDF</h3>
          <p className="text-sm text-obsidian-500">Tap to start editing metadata</p>
        </div>
      ) : pdfData.isLocked ? (
        ...
        <div className="space-y-6">
          <div className="bg-obsidian-50 dark:bg-obsidian-900 p-6 rounded-3xl border border-obsidian-200 dark:border-white/5 flex items-center gap-6 shadow-sm">
            <div className="w-16 h-16 bg-ember-50 dark:bg-ember-900/20 text-ember-500 rounded-2xl flex items-center justify-center shrink-0"><Info size={24} /></div>
          ...
          <div className="bg-obsidian-50 dark:bg-obsidian-900 p-8 rounded-[2rem] border border-obsidian-200 dark:border-white/5 space-y-6 shadow-sm">
            {!downloadUrl ? (
              <div className="space-y-6">
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-obsidian-500 mb-3 px-1">Output Filename</label>
                  <input 
                    type="text" 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {['title', 'author', 'subject', 'keywords', 'creator', 'producer'].map(field => (
                    <div key={field}>
                      <label className="block text-[10px] font-black uppercase text-obsidian-500 mb-2 tracking-widest px-1">{field}</label>
                      <input type="text" value={(meta as any)[field]} onChange={(e) => setMeta({...meta, [field]: e.target.value})} className="w-full bg-obsidian-100 dark:bg-obsidian-950 rounded-xl px-4 py-3 border border-transparent focus:border-ember-500 outline-none font-bold text-sm dark:text-white" />
                    </div>
                  ))}
                </div>
              </div>
