...
export default function WatermarkTool() {
  ...
  const ActionButton = () => (
    <button onClick={applyWatermark} disabled={isProcessing || !text} className={`w-full bg-ember-500 hover:bg-ember-600 text-white font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 py-4 rounded-2xl text-sm md:p-6 md:rounded-3xl md:text-xl flex items-center justify-center gap-3 shadow-lg shadow-ember-500/20`}>
    ...
    <NativeToolLayout title="علامة مائية" description="إضافة نص مخصص للعلامة التجارية أو الأمان." actions={pdfData && !pdfData.isLocked && !downloadUrl && <ActionButton />}>
      ...
      {!pdfData ? (
        <div onClick={() => !isProcessing && fileInputRef.current?.click()} className="border-4 border-dashed border-obsidian-200 dark:border-obsidian-900 rounded-[2.5rem] p-12 text-center hover:bg-ember-50 dark:hover:bg-ember-900/10 transition-all cursor-pointer group">
          <div className="w-20 h-20 bg-ember-50 dark:bg-ember-900/20 text-ember-500 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform"><Type size={32} /></div>
          <h3 className="text-xl font-bold dark:text-white mb-2">اختر ملف PDF</h3>
          <p className="text-sm text-obsidian-500">اضغط لإضافة علامة مائية</p>
        </div>
      ) : pdfData.isLocked ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in duration-500">
          <div className="lg:col-span-2 space-y-6">
            {/* Live Preview */}
            <div className="bg-obsidian-50 dark:bg-obsidian-900 p-6 rounded-[2rem] border border-obsidian-200 dark:border-white/5 shadow-sm overflow-hidden flex flex-col items-center">
               ...
               <div className="relative aspect-[3/4] w-full max-w-[300px] bg-obsidian-50 border border-obsidian-200 dark:border-obsidian-800 rounded-xl overflow-hidden shadow-inner">
                  {pdfData.thumbnail ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-obsidian-100"><Type size={64} /></div>
                  )}
                  <div 
          ...
          <div className="space-y-6">
            <div className="bg-obsidian-50 dark:bg-obsidian-900 p-8 rounded-[2rem] border border-obsidian-200 dark:border-white/5 shadow-sm space-y-6">
              {!downloadUrl ? (
                <>
                  <div>
                    <label className="block text-[10px] font-black uppercase text-obsidian-500 mb-3">نص العلامة المائية</label>
                    <input type="text" value={text} onChange={(e) => setText(e.target.value)} className="w-full bg-obsidian-100 dark:bg-obsidian-950 rounded-xl px-4 py-3 border border-transparent focus:border-ember-500 outline-none font-bold text-sm dark:text-white" />
                  </div>
                  
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                       <label className="text-[10px] font-black uppercase text-obsidian-500">المظهر</label>
                       <Palette size={14} className="text-obsidian-400" />
                    </div>
                    
                    <div className="grid grid-cols-2 gap-4">
                       <div className="col-span-2">
                          <label className="block text-[8px] font-black uppercase text-obsidian-500 mb-2">اللون</label>
                          <div className="flex gap-2 flex-wrap">
                             {['#D4891A', '#3B82F6', '#10B981', '#F59E0B', '#000000'].map(c => (
                               <button 
                                 key={c} 
                                 onClick={() => setColor(c)}
                                 className={`w-8 h-8 rounded-full border-2 transition-all ${color === c ? 'border-obsidian-900 dark:border-white scale-110' : 'border-transparent'}`}
                       ...
                       <div>
                          <label className="block text-[8px] font-black uppercase text-obsidian-500 mb-2">الشفافية ({Math.round(opacity * 100)}%)</label>
                          <input type="range" min="0.1" max="1" step="0.1" value={opacity} onChange={(e) => setOpacity(parseFloat(e.target.value))} className="w-full accent-ember-500" />
                       </div>
                       <div>
                          <label className="block text-[8px] font-black uppercase text-obsidian-500 mb-2">الحجم ({fontSize}px)</label>
                          <input type="range" min="10" max="200" step="1" value={fontSize} onChange={(e) => setFontSize(parseInt(e.target.value))} className="w-full accent-ember-500" />
                       </div>
                       <div>
                          <label className="block text-[8px] font-black uppercase text-obsidian-500 mb-2">الدوران ({rotation}°)</label>
                          <input type="range" min="-180" max="180" step="5" value={rotation} onChange={(e) => setRotation(parseInt(e.target.value))} className="w-full accent-ember-500" />
                       </div>
                    </div>
                  </div>
