...
export default function ProtectTool() {
  ...
  const [customFileName, setCustomFileName] = useState('paperknifeAR-protected')
  ...
  return (
    <NativeToolLayout title="حماية PDF" description="إضافة تشفير قوي لمستنداتك. تتم المعالجة محلياً." actions={pdfData && !pdfData.isLocked && !objectUrl && <ActionButton />}>
      ...
      {!pdfData ? (
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-gray-100 dark:border-white/5 flex items-center gap-6">
            ...
            <div className="p-4 bg-amber-50 dark:bg-amber-900/10 rounded-2xl border border-amber-100 dark:border-white/5 flex items-start gap-3"><Lock size={14} className="text-amber-500 shrink-0 mt-0.5" /><p className="text-[10px] text-amber-700 dark:text-amber-400 leading-relaxed font-medium">لا يمكن لـ PaperKnifeAR استعادة كلمات المرور المنسية. احفظها في مكان آمن.</p></div>
