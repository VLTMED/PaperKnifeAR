...
export default function RepairTool() {
  ...
  const startRepair = async () => {
    ...
    try {
      ...
      toast.success('تم إعادة بناء الملف بنجاح!')
    } catch (error: any) { 
  ...
  return (
    <NativeToolLayout title="إصلاح PDF" description="إصلاح ملفات PDF التالفة بإعادة بناء هيكلها." actions={originalFile && !downloadUrl && <ActionButton />}>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-obsidian-50 dark:bg-obsidian-900 p-8 rounded-[2rem] border border-obsidian-200 dark:border-white/5 space-y-8 shadow-sm">
            {!downloadUrl ? (
              ...
              <SuccessState message="اكتمل إعادة البناء!" downloadUrl={downloadUrl} fileName={`${customFileName}.pdf`} onStartOver={() => { setDownloadUrl(null); setOriginalFile(null); }} showPreview={true} />
            )}
          </div>
        </div>
      )}
      ...
      <div className="mt-8 p-6 bg-amber-50 dark:bg-amber-900/10 rounded-2xl border border-amber-100 dark:border-white/5 flex items-start gap-4">
        ...
        <div className="text-xs text-amber-700 dark:text-amber-400 leading-relaxed text-left">
          <p className="font-black mb-1 uppercase tracking-widest text-[10px]">البروتوكول التقني:</p>
          يُعيد PaperKnifeAR بناء جدول الإسناد الداخلي ويُجدد هيكل الملف من الصفر. هذا يمكنه استعادة الوصول إلى الملفات التي "لا يمكن فتحها".
        </div>
      </div>
      <PrivacyBadge />
    </NativeToolLayout>
