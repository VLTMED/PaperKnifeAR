...
const pathToKey: Record<string, { title: string; desc: string }> = {
  ...
  '/protect': { title: t('tool.protect.title', 'ar'), desc: t('tool.protect.desc', 'ar') },
  '/unlock': { title: t('tool.unlock.title', 'ar'), desc: t('tool.unlock.desc', 'ar') },
  '/rotate-pdf': { title: t('tool.rotate.title', 'ar'), desc: t('tool.rotate.desc', 'ar') },
  '/rearrange-pdf': { title: t('tool.rearrange.title', 'ar'), desc: t('tool.rearrange.desc', 'ar') },
  '/page-numbers': { title: t('tool.pageNumbers.title', 'ar'), desc: t('tool.pageNumbers.desc', 'ar') },
  '/watermark': { title: t('tool.watermark.title', 'ar'), desc: t('tool.watermark.desc', 'ar') },
  '/metadata': { title: t('tool.metadata.title', 'ar'), desc: t('tool.metadata.desc', 'ar') },
  '/signature': { title: t('tool.signature.title', 'ar'), desc: t('tool.signature.desc', 'ar') },
  '/grayscale': { title: t('tool.grayscale.title', 'ar'), desc: t('tool.grayscale.desc', 'ar') },
  '/pdf-to-image': { title: t('tool.pdfToImage.title', 'ar'), desc: t('tool.pdfToImage.desc', 'ar') },
  '/image-to-pdf': { title: t('tool.imageToPdf.title', 'ar'), desc: t('tool.imageToPdf.desc', 'ar') },
  '/extract-images': { title: t('tool.extractImages.title', 'ar'), desc: t('tool.extractImages.desc', 'ar') },
  '/pdf-to-text': { title: t('tool.pdfToText.title', 'ar'), desc: t('tool.pdfToText.desc', 'ar') },
  '/repair': { title: t('tool.repair.title', 'ar'), desc: t('tool.repair.desc', 'ar') },
}
...
export default function AndroidToolsView({ tools }: { tools: Tool[] }) {
  ...
  const getArabicTitle = (tool: Tool) => {
    if (tool.path && pathToKey[tool.path]) return pathToKey[tool.path].title
    return tool.title
  }

  const getArabicDesc = (tool: Tool) => {
    if (tool.path && pathToKey[tool.path]) return pathToKey[tool.path].desc
    return tool.desc
  }

  const filteredTools = useMemo(() => {
    return tools.filter(tool => {
      const arTitle = getArabicTitle(tool)
      const arDesc = getArabicDesc(tool)
      return arTitle.includes(searchQuery) ||
      ...
      <main className="px-4 space-y-8">
        {(Object.keys(groupedTools) as ToolCategory[]).map((category) => (
          <section key={category} className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <h3 className="px-2 mb-4 text-[11px] font-black uppercase tracking-[0.2em] text-[#49454F] dark:text-[#CAC4D0] text-right">
              أدوات {categoryNames[category]}
            </h3>
