    ...
    <div dir="rtl" className="min-h-screen bg-[#FAFAFA] dark:bg-black pb-32 transition-colors">
      <header className="px-6 pt-[calc(env(safe-area-inset-top)+1rem)] pb-6 sticky top-0 bg-[#FAFAFA]/90 dark:bg-black/90 backdrop-blur-xl z-50 border-b border-gray-100 dark:border-white/5">
        ...
        <div className="relative group">
          <div className="absolute inset-y-0 right-5 flex items-center pointer-events-none text-gray-400 group-focus-within:text-rose-500 transition-colors">
            <SearchIcon size={18} />
          </div>
          <input 
            type="text"
            ...
            dir="rtl"
            className="w-full bg-white dark:bg-zinc-900 border border-gray-100 dark:border-white/5 rounded-2xl py-4 pr-14 pl-6 text-sm font-bold placeholder:text-gray-400 focus:bg-white dark:focus:bg-zinc-800 shadow-sm outline-none transition-all dark:text-white text-right"
          />
          {searchQuery && (
            <button 
      ...
      <main className="px-4 py-6 space-y-2">
        {filteredHistory.length === 0 ? (
          <div className="py-24 text-center flex flex-col items-center animate-in fade-in duration-700">
            ...
            <p className="text-xs text-gray-500 dark:text-zinc-500 max-w-[200px] mt-2 font-medium leading-relaxed">ستظهر المستندات المعالجة على هذا الجهاز هنا مؤقتاً.</p>
          </div>
        ) : (
          filteredHistory.map((item) => (
            <div key={item.id} className="p-4 bg-white dark:bg-zinc-900 rounded-[2rem] border border-gray-100 dark:border-white/5 flex items-center gap-4 active:scale-[0.99] transition-all shadow-sm group">
              <div className="flex items-center gap-2 shrink-0">
                 ...
                 <ChevronLeftIcon size={16} className="text-gray-200 dark:text-zinc-800" />
              </div>
        ...
        <div className="pt-12 flex flex-col items-center gap-3 pb-10 opacity-30">
           <div className="flex items-center gap-2">
             ...
             <span className="text-[8px] font-black uppercase tracking-[0.2em] text-gray-500">بروتوكول الخصوصية</span>
           </div>
